...
class Six : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val databaseReference = FirebaseDatabase.getInstance().getReference("zSix")
            ...
            override fun onChildAdded(p0: DataSnapshot, p1: String?) {
                ...
                myListView.onItemClickListener = AdapterView.OnItemClickListener { _, _, position, _ ->
                    ...
                    if(clickCounter % 4 == 0){
                        ...
                        val intent = Intent(this@Six, MainDetail::class.java)
                        ...
                        try{
                            ...
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                                startActivity(intent, ActivityOptions.makeSceneTransitionAnimation(this@Six).toBundle())
                            }
                            else {
        ...
        databaseReference.addValueEventListener(object : ValueEventListener {
            ...
            override fun onDataChange(p0: DataSnapshot) {
                for (player in p0.children) {
                    println(player.key)
                    Log.i("zSix", player.key.toString())
                    mainLister.add(player.toString())
                }
                so()
            }
            override fun onCancelled(p0: DatabaseError) = Unit
